...
import { useMarkPmbNotificationsRead, usePmbNotifications } from '@/hooks/usePmb'
...
export function PmbNotificationBell({ className }: { className?: string }) {
  const navigate = useNavigate()
  const isDesktop = useIsDesktopLg()
  const [open, setOpen] = useState(false)
  const { data, isLoading } = usePmbNotifications()
  const markRead = useMarkPmbNotificationsRead()
  ...
  const handleItemClick = (item: PmbNotificationItem) => {
    ...
    if (item.registration_uuid) {
      ...
      navigate(`/pmb/portal/pendaftaran/${item.registration_uuid}${hash}`)
    }
  }
  ...
  const triggerButton = (
    <Button
      ...
      variant="outline"
      size="icon"
      className={cn('relative size-11 shrink-0 border-primary/20', className)}
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      >
        <SheetTitle className="sr-only">Notifikasi</SheetTitle>
